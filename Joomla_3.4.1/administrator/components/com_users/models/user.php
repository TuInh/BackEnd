...
class UsersModelUser extends JModelAdmin {
	...
	public function isSuperUser()
	{
		...
		$postion = $this->_getUserDisplayedGroups($user_id);
		if ( strpos($postion,'Super Users') !== false) {
			return true;
		}
		else return false;
	...
	public function  isMemberofTeam($memberid)
	{
		...
		$messages = $db->loadObject()->teamid;
		
		$team_id = JFactory::getUser()->teamid;
		if($messages == $team_id)
			return true;
		else return false;
	...
	public function getItemByIds($pks) {
		...
		try {
			$db = $this->getDbo ();
			$db = JFactory::getDbo ();
			$query = $db->getQuery ( true )->select ( '*' )->from ( $db->quoteName ( '#__users' ) );
			$query->where ( 'id IN (' . implode ( ',', $pks ) . ')' );
			$db->setQuery ( $query );
				
			$usernamelst = $db->loadObjectList ();
			foreach ($usernamelst as $eachUsername)
			{
				$eachUsername->issue = array();
				$eachUsername->teamname = $this->getTeamName($eachUsername->teamid)->name;
				$eachUsername->projectnameInvolve = $this->getProjectname ($eachUsername->id )['projectlstname'];
				$eachUsername->position = $this->_getUserDisplayedGroups($eachUsername->id );
				$projectinvolve = $this->getProjectInvlove ($eachUsername->id );
				
				foreach ($projectinvolve as $proid)
				{
					$eachUsername->mandate[]= $this->getMandateEachMember ( $eachUsername->id, $proid );
					$eachUsername->issue[]= $this->getAllBinaryinProject ( $proid, $eachUsername->id );
					
				}
			//	echo "<pre>";	var_dump($eachUsername); echo "</pre>";die();
			}
	...
	public function getBinaryInvolve($userId) {
		$db = JFactory::getDBO ();
		$query = $db->getQuery ( true );
		$query->select ( 'binary_id' );
	...
	public function getProjectInvlove($userId) {
		$db = JFactory::getDBO ();
		
		$binarylst = $this->getBinaryInvolve ( $userId );
		
		$projectlst = array ();
		foreach ( $binarylst as $eachbinary ) {
			
			// var_dump($eachbinary);
			$querybianry = $db->getQuery ( true );
			$querybianry->select ( 'project_id' );
			$querybianry->from ( '#__binary' );
			$querybianry->where ( 'id = ' . $eachbinary->binary_id );
			
			$db->setQuery ( ( string ) $querybianry );
			
			$project_id = $db->loadObject ()->project_id;
			$flag = 0;
			for($x = 0; $x < count ( $projectlst ); $x ++) {
				if ($project_id == $projectlst [$x]) {
					$flag = 1;
					break;
				}
			}
			if ($flag == 0)
				$projectlst [] = $project_id;
		}
		
		return $projectlst;
	}
	public function getMandateEachMember($userId, $projectid) {
		$binarylst = $this->getBinaryInvolve ( $userId );
		$db = JFactory::getDBO ();
		...
		foreach ( $binarylst as $eachbinary ) {
			...
			$query->from ( '#__binary' );
			$query->where ( 'id = ' . $eachbinary->binary_id . ' AND ' . 'project_id = ' . $projectid );
			$db->setQuery ( ( string ) $query );			
			$publish = $db->loadObject ();
			if ($publish != NULL) {
				
				
				$datediff = strtotime ( $publish->publish_down ) - strtotime ( $publish->publish_up );
				$mandateeachBianary = floor ( $datediff / (60 * 60 * 24) );
				
				$mandate += $mandateeachBianary;
			}
		}
		
		return $mandate;
	}
	public function getProjectname($userId) {
		$projectlst = $this->getProjectInvlove ( $userId );
		
		$projectlstInvolve = array(
			'projectlstname' => array(),
			'projectlstid' => array()
		);
		...
		foreach ( $projectlst as $eachproject ) {
			...
			$queryproject = $db->getQuery ( true );
			$queryproject->select ( 'id,name' );
			$queryproject->from ( '#__project' );
			$queryproject->where ( 'id = ' . $eachproject );
			
			$db->setQuery ( ( string ) $queryproject );
			
			$object = $db->loadObject ();
			$projname = $object->name;
			$projectlstname [] = $projname;
			$projectlstid[] = $object->id;
		}
		
		$projectlstInvolve['projectlstname'] = $projectlstname;
		$projectlstInvolve['projectlstid'] = $projectlstid;
		return $projectlstInvolve;
	}
	public function getAllBinaryinProject($projectId, $user_id) {
		$db = JFactory::getDBO ();
		$query = $db->getQuery ( true );
		$query->select ( 'id' );
		$query->from ( '#__binary' );
		...
		for($i = 0; $i < count ( $binarylst ); $i ++) {
			...
			$queryIssueStt->where ( 'binary_id = ' . $binarylst [$i]->id . ' AND ' . 'user_id = ' . $user_id );
	...
	public function save($data) {
		...
		if (array_key_exists ( 'twofactor', $data )) {
			...
			if ($twoFactorMethod != 'none') {
				...
				$otpConfigReplies = FOFPlatform::getInstance ()->runPlugins ( 'onUserTwofactorApplyConfiguration', array (
						$twoFactorMethod 
				) );
		...
		foreach ( $pks as $i => $pk ) {
			if ($table->load ( $pk )) {
				...
				if ($allow) {
					...
					$dispatcher->trigger ( $this->event_before_delete, array (
							$table->getProperties () 
					) );
					...
					if (! $table->delete ( $pk )) {
						...
						$dispatcher->trigger ( $this->event_after_delete, array (
	...
	public function batchReset($user_ids, $action) {
		...
		$query->update ( $db->quoteName ( '#__users' ) )->set ( $db->quoteName ( 'requireReset' ) . ' = ' . $value )->where ( $db->quoteName ( 'id' ) . ' IN (' . implode ( ',', $user_ids ) . ')' );
	...
	public function getGroups() {
		...
		if ($user->authorise ( 'core.edit', 'com_users' ) && $user->authorise ( 'core.manage', 'com_users' )) {
			$model = JModelLegacy::getInstance ( 'Groups', 'UsersModel', array (
					'ignore_request' => true 
			) );
	 ...
	 *        	The user ID to retrieve the groups for
	 *        	
	...
	/**
	 * Returns the one time password (OTP) – a.k.a.
	...
	public function getOtpConfig($user_id = null) {
		...
		/*
		 * If the decryption failed for any reason we essentially disable the two-factor authentication. This prevents impossible to log in sites if the site admin changes the site secret for any reason.
		 */
	...
	/**
	 * Sets the one time password (OTP) – a.k.a.
	 * two factor authentication –
	...
	/**
	 * Gets the symmetric encryption key for the OTP configuration data.
	 * It
	 ...
	 *
	 * @param integer $user_id
	 *        	The user ID to load the forms for (optional)
	 *        	
	 * @return array
	 *
	 * @since 3.2
	...
	public function getTwofactorform($user_id = null) {
		...
		return FOFPlatform::getInstance ()->runPlugins ( 'onUserTwofactorShowConfiguration', array (
				$otpConfig,
				$user_id 
		) );
	}
	 ...
	 *        	The user ID
	 * @param integer $count
	 *        	How many OTEPs to generate? Default: 10
	 *        	
	 ...
	 * secret key.
	 * If not, it will check it against the list of one time
	...
	public function isValidSecretKey($user_id, $secretkey, $options = array()) {
		...
		if (empty ( $otpConfig->method ) || ($otpConfig->method == 'none')) {
			...
			$lang->load ( $extension, JPATH_ADMINISTRATOR, null, false, true ) || $lang->load ( $extension, $source, null, false, true );
		...
		$otpAuthReplies = FOFPlatform::getInstance ()->runPlugins ( 'onUserTwofactorAuthenticate', array (
				$credentials,
				$options 
		) );
		...
		/*
		 * This looks like noob code but DO NOT TOUCH IT and do not convert to in_array(). During testing in_array() inexplicably returned null when the OTEP begins with a zero! o_O
		 */
	 ...
	 * (OTEP) for this user.
	 * If it is it will be automatically removed from the
	...
	public function isValidOtep($user_id, $otep, $otpConfig = null) {
		...
		if (empty ( $otpConfig->otep )) {
			if (empty ( $otpConfig->method ) || ($otpConfig->method == 'none')) {
				...
				/**
				 * Two factor authentication enabled and no OTEPs defined.
				 * The
		...
		if (in_array ( $otep, $otpConfig->otep )) {
			...
			$otpConfig->otep = array_diff ( $otpConfig->otep, array (
					$otep 
			) );
