...
class ProjectControllerProject extends JControllerForm
{
	...
	public function submit()
	{
		...
		if ($params->get('validate_session', 0))
		{
			...
			{
				JError::raiseWarning(403, JText::_('COM_PROJECT_SESSION_INVALID'));

				// Save the data in the session.
				$app->setUserState('com_project.project.data', $data);

				// Redirect back to the project form.
				$this->setRedirect(JRoute::_('index.php?option=com_project&view=project&id=' . $stub, false));
		...
		{
			$msg = JText::_('COM_PROJECT_EMAIL_THANKS');
		}
		else
		{
			$msg = '';
		...
		$app->setUserState('com_project.project.data', null);
		...
		{
			$this->setRedirect(JRoute::_('index.php?option=com_project&view=project&id=' . $stub, false), $msg);
		}
	...
	private function _sendEmail($data, $project, $copy_email_activated)
	{
			...
			// Prepare email body
			$prefix = JText::sprintf('COM_PROJECT_ENQUIRY_TEXT', JUri::base());
			$body	= $prefix . "\n" . $name . ' <' . $email . '>' . "\r\n\r\n" . stripslashes($body);
			...
			{
				$copytext    = JText::sprintf('COM_PROJECT_COPYTEXT_OF', $project->name, $sitename);
				$copytext    .= "\r\n\r\n" . $body;
				$copysubject = JText::sprintf('COM_PROJECT_COPYSUBJECT_OF', $subject);
